...
//'estudiante','docente', 'directtivvo', 'admin','egresado',
	...
	$s = sql_select('login, htpass, statut', 'spip_auteurs', sql_in('statut', array('1comite', '0minirezo', 'estudiante','docente', 'directtivvo', 'admin','egresado', 'nouveau')));
	while ($t = sql_fetch($s)) {
		if (strlen($t['login']) and strlen($t['htpass'])) {
			$p1 .= $t['login'] . ':' . $t['htpass'] . "\n";
			if (($t['statut'] == '0minirezo') or ($t['statut'] == 'admin')) {
				$p2 .= $t['login'] . ':' . $t['htpass'] . "\n";
			}
		}
	}
